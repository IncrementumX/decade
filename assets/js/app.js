...
;(function(){if(window.__cbLightbox)return;window.__cbLightbox=1;function build(){var m=document.createElement('div');m.id='cbLightbox';m.style.cssText='display:none;position:fixed;inset:0;z-index:100000;background:rgba(15,23,42,.62);align-items:center;justify-content:center;padding:24px';m.innerHTML='<div style="background:#fff;border-radius:14px;max-width:1120px;width:100%;max-height:92vh;overflow:auto;padding:20px 24px;position:relative;box-sizing:border-box"><button id="cbClose" title="Close" style="position:absolute;top:12px;right:14px;font-size:20px;line-height:1;color:#5d6b7d;background:#f1f5fa;border:1px solid #dde4ec;border-radius:8px;width:32px;height:32px;cursor:pointer">&times;</button><div id="cbTitle" style="font-size:14px;font-weight:800;color:#1b2430;margin:2px 44px 14px 0"></div><div id="cbBody"></div></div>';document.body.appendChild(m);function close(){m.style.display='none';document.getElementById('cbBody').innerHTML='';}m.addEventListener('click',function(e){if(e.target===m)close();});document.getElementById('cbClose').onclick=close;document.addEventListener('keydown',function(e){if(e.key==='Escape')close();});return m;}var modal=build();document.querySelectorAll('.cb-zoom').forEach(function(btn){btn.addEventListener('click',function(){var fig=btn.closest('figure');if(!fig)return;var svg=fig.querySelector('svg');if(!svg)return;var cl=svg.cloneNode(true);cl.removeAttribute('width');cl.removeAttribute('height');cl.style.width='100%';cl.style.height='auto';var t=fig.querySelector('.charttitle');document.getElementById('cbTitle').textContent=t?t.textContent.trim():'';var b=document.getElementById('cbBody');b.innerHTML='';b.appendChild(cl);modal.style.display='flex';});});})();
